...
class CallbackSuccessHandlePayoutAction
{
    ...
    public function execute(array $payloadRaw): void
    {
        ...
        if (!$ref) {
            Log::error('Missing payout reference in success payout callback. Payload keys: ' . json_encode(array_keys($payloadRaw)));
            return;
        }
        ...
        $this->repository->updatePayoutByPayoutNumber($ref, $data);
    }
}